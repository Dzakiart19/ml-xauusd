...
class XAUModel:
    ...
    def simulate_labels(df: pd.DataFrame) -> pd.Series:
        """
        Untuk setiap candle, simulasikan apakah trade BUY menang (1) atau kalah (0).
    ...
    def initial_train(self, df: pd.DataFrame):
        ...
        try:
            ...
            scores = cross_val_score(self.model, X_scaled, y_train, cv=3, scoring='accuracy')
            acc = scores.mean()
    ...
    def retrain(self, trades: list) -> float:
        ...
        for t in trades:
            ...
            for feat in feature_names:
                # Mapping nama kolom DB → nama fitur
                col_map = {
    ...
    def predict(self, feature_row: pd.Series) -> tuple:
        """
        Kembalikan (label: int, proba: float).
        label 1 = BUY, 0 = SELL
        proba = probabilitas label yang diprediksi
        """
        if self.model is None or not hasattr(self.model, 'estimators_'):
            return 1, 0.5   # default BUY jika model belum dilatih
