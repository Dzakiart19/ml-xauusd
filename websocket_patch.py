...
_INIT_CONTENT = '''"""websocket-client __init__.py (auto-restored by websocket_patch)"""
...
def _find_websocket_client_path() -> str | None:
    """Cari direktori websocket-client di sys.path."""
    for p in sys.path:
...
def ensure_websocket_client():
    ...
    Jika tidak, buat/perbarui __init__.py secara otomatis.
    """
    try:
        # Coba import & cek atribut
        ...
        # Tidak ada WebSocketApp — perlu patch
        ws_path = _find_websocket_client_path()
        if not ws_path:
            logger.error("websocket-client tidak ditemukan di sys.path.")
            return
        ...
        if hasattr(_ws2, 'WebSocketApp'):
            ...
            logger.error("Patch websocket-client gagal.")
